...
def pick_sixty_years_chance(index):
    ...
    link = 'http://'
    chanceDict['url'] = 'www.chance.org.tw/籤詩集/%s/籤詩網‧%s__第%02d籤.htm' %(godname, godname, index)
    r = rq.get(link + chanceDict['url'])
    r.encoding = 'big5'
    #顯示網頁狀態
    # print(r.status_code)
    #顯示200即為正常
    ...
    for i in imgs:
        if i['src'].rfind('.jpg') >= 0:
            chanceDict['image'] = 'www.chance.org.tw/籤詩集/%s/' %godname
